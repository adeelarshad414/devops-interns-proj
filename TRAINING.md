...
| **API gateway** | Edge policy: auth, rate limiting, keys/quotas, WAF, versioning | nginx is the lightweight gateway (routing + `limit_req`); on AWS a **managed HTTP API Gateway** ([`infra/aws/apigateway.tf`](infra/aws/apigateway.tf)) fronts the ALB via a VPC Link with per-route throttling + access logs, and a **WAFv2** web ACL sits on the ALB ([`infra/aws/edge.tf`](infra/aws/edge.tf)). GCP API Gateway / Azure APIM are the equivalents there. |
