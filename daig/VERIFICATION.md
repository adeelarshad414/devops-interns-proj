...
| YAML — compose ×3, workflows ×4, k8s, observability, ansible, swarm | VERIFIED (static) | PyYAML `safe_load_all`, every document loads |
...
| Dockerfiles ×4 | **UNVERIFIED** | no Docker daemon available |
| `docker compose up` end to end | **UNVERIFIED** | no Docker daemon |
| Kubernetes manifests | **UNVERIFIED** | no cluster, no `kubectl` |
...
1. **OpenTelemetry JS package versions.** This ecosystem churns faster than
   ...
   thing to break `npm install`.
2. **`npm install` inside the Docker build** — pinned versions may have moved.
