...
Application/runtime code still has not been executed end to end. What the audit
changes added **was** machine-checked: `kubectl kustomize k8s/base` builds
cleanly, `docker compose config` passes for base + observability, all workflow
YAML parses, and the lockfiles were generated by npm. See `VERIFICATION.md` for
the full split. Before the first cohort, someone must still run the stack.
